                                        ...
                                        @foreach ($banners as $key => $banner)
                                            <tr>
                                                ...
                                                <td width="30%">
                                                    @if (!empty($banner['banner_image']))
                                                        <img style="width: 60px;"
                                                            ...
                                                            alt="{{ $banner->title }}">
                                                    @else
                                                        <img style="width: 60px; border: 3px solid red"
                                                            src="{{ url('/storage/banner/no_image.png') }}"
                                                            alt="No Image">
                                                    @endif
                                                </td>
                                                <td>
                                                    @if ($banner->status == 1)
                                                        <a title="Change" banner_id="{{ $banner->id }}"
                                                            class="text-success banner_status"
                                                            id="banner_{{ $banner->id }}" href="javascript:void(0)">
                                                            Active
                                                        </a>
                                                    @else
                                                        <a title="Change" banner_id="{{ $banner->id }}"
                                                            class="banner_status text-danger"
                                                            id="banner_{{ $banner->id }}" href="javascript:void(0)"> In
                                                            Active
                                                        </a>
                                                    @endif
                                                </td>
                                                <td>
                                                    ...
                                                    <a href="#deleteModal{{ $banner->id }}" data-toggle="modal"
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            $(".banner_status").click(function() {
                ...
                $.ajax({
                    type: 'post',
                    url: '/sadmin/update-banner-status',
                    data: {
                        status: status,
                        banner_id: banner_id
                    },
